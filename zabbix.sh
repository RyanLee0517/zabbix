...
grant all privileges on zabbix.* to zabbix@localhost identified by 'zabbixambsoft';
...
sudo zcat /usr/share/zabbix-sql-scripts/mysql/server.sql.gz | mysql --default-character-set=utf8mb4 -u zabbix -p'zabbixambsoft' zabbix
...
sed -i 's/# DBPassword=/DBPassword=zabbixambsoft/g' /etc/zabbix/zabbix_server.conf
# Configure SNMP Ping
...
sed -i '395 i [ zabbix.ambsoft.local ]' /etc/ssl/openssl.cnf
sed -i '396 i subjectAltName = DNS:zabbix.ambsoft.local' /etc/ssl/openssl.cnf
...
ambsoft
zabbix
zabbix.ambsoft.local
zabbix@zabbix.ambsoft.local
zabbixambsoft
ambsoft
EOF

# 142 + 143 Same Line
openssl x509 -in /etc/ssl/private/zabbix.csr -out /etc/ssl/private/zabbix.crt -req -signkey /etc/ssl/private/zabbix.key 
-extfile /etc/ssl/openssl.cnf -extensions zabbix.ambsoft.local -days 3650
...
sudo cat << EOF > /etc/apache2/sites-available/zabbix.ambsoft.local.conf 
<VirtualHost *:80> 
    ServerName zabbix.ambsoft.local
    ServerAlias www.zabbix.ambsoft.local
    Redirect permanent / https://zabbix.ambsoft.local
</VirtualHost>
...
<VirtualHost *:443>
    ...
    ServerName zabbix.ambsoft.local
    ServerAlias www.zabbix.ambsoft.local
    ServerAdmin admin@zabbix.ambsoft.local
    DocumentRoot /usr/share/zabbix

    ErrorLog ${APACHE_LOG_DIR}/www.zabbix.ambsoft.local_error.log
    CustomLog ${APACHE_LOG_DIR}/www.zabbix.ambsoft.local_access.log combined
...
sudo a2ensite zabbix.ambsoft.local.conf
